...
export class DeckView extends ItemView {
  ...
  // Replace Wikilinks with standard markdown image syntax
  replaceImageWikilinks(markdown: string): string {
    const wikilinkRegex = /!\[\[(.+?)\]\]/g;
    return markdown.replace(wikilinkRegex, (_, name) => {
      const url = this.app.vault.adapter.getResourcePath(name);
      return `![${name}](${url})`;
    });
  }

  // Replace /// directive shorthand with Marp HTML comments
  // This must be done before marp.render() because Marp parses directives
  // from raw markdown, not from HTML generated by markdown-it plugins
  replaceMarpDirectives(markdown: string): string {
    // Match lines starting with /// followed by content
  ...
  async renderPreview() {
    ...
    // Step 2: Convert /// directive shorthand to Marp HTML comments
    if (this.settings.enableMarkdownItPlugins) {
      content = this.replaceMarpDirectives(content);
    }

    // Step 3: Convert Mermaid code blocks to inline SVGs
