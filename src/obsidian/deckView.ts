...
export class DeckView extends ItemView {
  ...
  async renderPreview() {
    ...
    // Note: innerHTML is required here as Marp generates complex HTML/SVG output
    // that cannot be practically built with DOM API. The content is generated by
    // @marp-team/marp-core from the user's own markdown, not external input.
    const contentEl = this.slidesContainerEl.createDiv();
    // eslint-disable-next-line no-unsanitized/property -- Marp-generated HTML from user's own markdown
  ...
  scrollToSlide(slideIndex: number, forceScroll = false) {
    try {
      ...
      console.debug('Preview slide not found');
    }
  }
  ...
  private attachSlideClickHandlers() {
    ...
    slides.forEach((slide, index) => {
      // Cursor is set via CSS: .marp-ext-deck-slides [data-marp-vscode-slide-wrapper]
  ...
  // Active slide styles are now in styles.css
  // This method is kept for compatibility but no longer creates style elements
  private ensureActiveSlideStyles() {
    // Styles are defined in styles.css: [data-marp-vscode-slide-wrapper].marp-ext-active-slide
  }
  ...
  private updateToolbarVisibility() {
    if (this.isInSidebar()) {
      this.toolbarEl.removeClass('hidden');
    } else {
      this.toolbarEl.addClass('hidden');
    }
  }
  ...
  private createToolbarButtons() {
    ...
    this.createToolbarButton('download', 'Export as PDF', () => {
      if (this.file)
        void exportSlide(this.app, this.file, 'pdf', this.settings.themeDir, this.getExportOptions());
    });

    this.createToolbarButton('image', 'Export as PPTX', () => {
      if (this.file)
        void exportSlide(this.app, this.file, 'pptx', this.settings.themeDir, this.getExportOptions());
    });

    this.createToolbarButton('code-glyph', 'Export as HTML', () => {
      if (this.file)
        void exportSlide(this.app, this.file, 'html', this.settings.themeDir, this.getExportOptions());
    });
  }
  ...
  openSearch() {
    ...
    this.searchContainerEl!.addClass('visible');
  ...
  closeSearch() {
    if (this.searchContainerEl) {
      this.searchContainerEl.removeClass('visible');
    }
  ...
  isSearchOpen(): boolean {
    return (
      ...
      this.searchContainerEl.hasClass('visible')
    );
  }
  ...
  private createSearchUI() {
    ...
    // Create search bar container (styles via CSS class)
    this.searchContainerEl = container.createDiv({ cls: 'marp-ext-search-bar' });

    // Search input (styles via CSS class)
    ...
    // Results counter (styles via CSS class)
